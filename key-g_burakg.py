...
def clear():
      if os.name=='nt':
          os.system('cls && title [NEW] Key - G - FULL KEY GENERAT0R BY BurakG ')
      else :
          os.system('clear')
#------------#
def strounga(x,y):
...
logo=choice(["""{w}
   ^     . .     ^  
   \ \    V    / /  
   \   \(+ +)/   / {r}({y}Instagram{r}){rr}~~~~~~~~~{r}({y}@whoisburakg{r}){w} 
   \   (  v  )   / {r}({y}Telegram{r}){rr}~~~~~~~~~~{r}({y}@burakgresmi{r}){w}
...
{rr}*{r} Instagam     {rr}:{y}@whoisburakg
{rr}*{r} Telegram     {rr}:{y}@burakgresmi
{rr}*{r} Tüm İletişim Adreslerim {rr}:{y}linktr.ee{rr}/{y}burak_g
{rr}*{r} Son Güncelleme  {rr}:{y}Nov{rr}/{y}2022


""".format(r=la7mar,y=lasfar,rr=ramadi,w=labyadh),
"""                                                   {y} 
  ____  __ __ ____   ___  __ __      ___ 
 || )) || || || \\ // \\ || //     // \\
 ||=)  || || ||_// ||=|| ||<<     (( ___
 ||_)) \\_// || \\ || || || \\     \\_||

{rr}\\_{r}Instagram{rr}>{y}@whoisburakg
{rr}\\_{r}Telegram{rr}>{y}@burakgresmi
{rr}\\_{r}Youtube{rr}>{y}Burak{rr}/{y}G
{rr}\\_{r}Son Güncellemeb{rr}:{y}Nov{rr}/{y}2022
...
class ccSama(object):
  ...
  $$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$"""%(blid,lasfar,lazra9,lasfar,lazra9,lasfar,la7mar,lazra9,lasfar,la7mar,lazra9,lasfar,la7mar,lazra9,lasfar))
    def gen1(self ,Sa7by):#Sa7by Kol Chy Wadha7 W jawna Bahy Brcha xDD...
      ...
      with open('Key-G Sonuçlar/ccgenerated.txt','a') as escnoor:
      ...
      print(" %s[%s%s Kaydedildi !  'Key-G Sonuçlar/ccgenerated.txt'%s]%s"%(la7mar,lasfar,mouchi,la7mar,labyadh))
...
class ghrab(object):
    def __init__(self):
        ...
        for ip in ip_range:
            xXx=xXx+1
            with open('Key-G Sonuçlar/Ipgenerated.txt', 'a') as xX:
                xX.write(str(ip) + ':' + str(PRoxYPort) + '\n')
        print("%s[%s%s Kaydedildi ! 'Key-G Sonuçlar/Ipgenerated.txt'%s]%s"%(la7mar,lasfar,xXx,la7mar,labyadh))
        so=input("%sContinue %s? %s[%sY%s/%sN%s(%syes%s/%sNo%s)] %s>%s "%(lasfar,ramadi,la7mar,lasfar,la7mar,lasfar,la7mar,lasfar,la7mar,lasfar,la7mar,ramadi,labyadh))
        if so.upper() in ['Y','YES','AY']:
            z4ck()
        else:
...
class z4ck(object):
  ...
  def __init__(self):
      ...
      print(strounga(xnumx+1,'Kredi Kartı'))
      print(strounga(xnumx+2,'Telefon Numarası'))
      print(strounga(xnumx+3,'Proxy'))
      print(strounga(xnumx+4,'Facebook AcessToken'))
      print(strounga(xnumx+5,'Kendi jeneratörünüzü yaratın'))
      print(strounga(xnumx+6,'Bir yapılandırmayı sil'))
      print(strounga(xnumx+7,'Hakkında (coded by burakg)'))
      baka=input('\033[91m[\033[93mBurakG>\033[91m]\033[00m ')
      while baka=='':
        baka=input('\033[91m[\033[93mBurakG>\033[91m]\033[00m ')
      baka=int(baka)
      while baka <1 or baka >(xnumx+7):
         baka=int(input('\033[91m[\033[93mBurakG>\033[91m]\033[00m '))
      if baka<=xnumx:
          ma=int(input('\033[91m[\033[93mNe Kadar\033[91m]\033[00m '))
          while ma <1:
           ma=int(input('\033[91m[\033[93mNe Kadar\033[91m]\033[00m '))
          for i in range(ma):
             conf=configs[baka-1]
             print(str(self.x1x(conf['format'])))
             with open('Key-G Sonuçlar/%s.txt'%(conf['name']),'a') as L:
               L.write('%s\n'%(self.x1x(conf['format'])))
          print(" %s[%s%s Kaydedildi ! 'Key-G Sonuçlar/%s.txt' %s]%s"%(la7mar,lasfar,ma,conf['name'],la7mar,labyadh))
          so=input("%sContinue %s? %s[%sY%s/%sN%s(%syes%s/%sNo%s)] %s>%s "%(lasfar,ramadi,la7mar,lasfar,la7mar,lasfar,la7mar,lasfar,la7mar,lasfar,la7mar,ramadi,labyadh))
          if so.upper() in ['Y','YES','AY']:
              z4ck()
          else:
...
#1,2,3 Go!
  ...
  def x1x(self,x):
   ...
   return x444
  def phnnm(self):
    if not os.path.isdir('Key-G Sonuçlar/phonz'):
     os.mkdir('Key-G Sonuçlar/phonz')
    print("""
        Helper:
    ...
    for i in range(Hwmch):
      ...
      ousm='+'+self.x1x(bakas)
      print(ousm)
      with open('Key-G Sonuçlar/phonz/C0de_%s.txt'%(str(intcd)),'a') as L:
        L.write('%s\n'%ousm)
    print(" %s[%s%s Kaydedildi ! 'Key-G Sonuçlar/phonz/C0de_%s.txt'%s]%s"%(la7mar,lasfar,Hwmch,str(intcd),la7mar,labyadh))
    so=input("%sContinue %s? %s[%sY%s/%sN%s(%syes%s/%sNo%s)] %s>%s "%(lasfar,ramadi,la7mar,lasfar,la7mar,lasfar,la7mar,lasfar,la7mar,lasfar,la7mar,ramadi,labyadh))
    if so.upper() in ['Y','YES','AY']:
           z4ck()
    else:
...
#fbtokengen
  def genetok(self):
   ...
   for az in range(int(zz)):
    ...
    print(escnoor)
    with open('Key-G Sonuçlar/tokens.txt','a') as ttt:
      ttt.write(str(escnoor)+'\n')
   print(" %s[%s%s Kaydedildi ! 'Key-G Sonuçlar/tokens.txt'%s]%s"%(la7mar,lasfar,zz,la7mar,labyadh))
   so=input("%sContinue %s? %s[%sY%s/%sN%s(%syes%s/%sNo%s)] %s>%s "%(lasfar,ramadi,la7mar,lasfar,la7mar,lasfar,la7mar,lasfar,la7mar,lasfar,la7mar,ramadi,labyadh))
   if so.upper() in ['Y','YES','AY']:
            z4ck()
   else:
...
#Key creator 
  ...
  def seemydick(self):
   print("""{r}
     ({y}Name/Nickname{r}){rr}~~{r}({y}Burak Gökkaya{rr}/{y}BurakG{r})
     ({y}Youtube{r}){rr}~~~~~~~~{r}({y}https{rr}://{y}www.youtube.com{rr}/{y}/channel/UCDGmtttS043h22WNFuE6XnA{r})
     ({y}Versiyon{r}){rr}~~~~~~~~{r}({y}2.0{r})
     ({y}Son Güncelleme{r}){rr}~~~~~~~~{r}({y}Nov-2022{r})
     {rr}*{y} Tool'u Çalmayın {rr}..{y} Boşuna Uğraşmayın Benim Gibi Olamazsınız
     {rr}*{y} Merhaba Ben BurakG 
     {rr}*{y} Bu Tool Tamamiyle Burak Gökkaya Tarafından Kodlanmıştır {rr}..{y} SAYGILARIMLA 
     {rr}*{y} Bazı Keyler Çalışmayabilir {rr}..{y} Üzgünüm {rr};) 
     {rr}*{y} Coded by Burak Gökkaya {rr}...""".format(r=la7mar,y=lasfar,rr=ramadi))
#------------#    
try:
  if not os.path.isdir('Key-G Sonuçlar'):
    os.mkdir('Key-G Sonuçlar')
  if not os.path.isfile('config.ini')or len(open('config.ini','r').read())<1:
   ...
   for f in open('config.ini','r').read().splitlines():
    if '::' in f:
      ...
      xnumx+=1
   for i in "[/] [%s] Configler Yükleniyor.. Coder By Burak Gökkaya ... SAYGILARIMLA !"%(xnumx):
          sys.stdout.write(i)
          sys.stdout.flush()
          time.sleep(0.1)
   z4ck()
